...
SELECT COUNT(DISTINCT id) AS UniqueAppID FROM AppData;  
...
SELECT prime_genre, COUNT(*) AS NumApps FROM AppData GROUP BY prime_genre ORDER BY NumApps DESC;
...
-- Market Insights -- Say we have an investor persona looking to invest in iOS app market what are insights that our client cana make data driven decisions from?
...
-- AVerage price distribution within different genres - Helps to decide the pricing of an app, if it's paid.
